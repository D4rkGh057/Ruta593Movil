...
export class AuthApiAdapter implements LoginPort {
    async login(email: string, password: string): Promise<{ user: User; token: string }> {
        try {
            ...
            else if (profileData.numero_identificacion)
                console.log("     ✅ Campo numero_identificacion de la API");
            else if (profileData.id)
                console.log("     ⚠️ Usando últimos 8 dígitos del ID como fallback");
            ...
            if (user.identificacion && user.identificacion.trim() !== "") {
                ...
                try {
                    const SessionStorage = (await import("../../adapters/stores/SessionStorage"))
                        .default;
